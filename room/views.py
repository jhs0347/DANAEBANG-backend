...
class NearInfoView(View):
    def get(self, request):
        ...
        try:
            ...
            convenience_infos      = ConvenienceInfo.objects.filter(condition)
            near_convenience_infos = [info for info in convenience_infos
                                      if haversine(position, (info.latitude, info.longitude)) <= 2]

            safety_infos      = SafetyInfo.objects.filter(condition)
            near_safety_infos = [info for info in safety_infos
                                 if haversine(position, (info.latitude, info.longitude)) <= 2]

            education_infos       = EducationInfo.objects.filter(condition)
...
class RoomListView(View):
    ...
    def get(self, request):
        ...
        try:
            ...
            if like:
                room_id_list = [room.id for room in (
                    Room.objects.filter(room_like = request.user.id)
                )]
            else:
